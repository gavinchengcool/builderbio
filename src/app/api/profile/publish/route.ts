...
const publishSchema = z.object({
  ...
  inferred_style_theme: z.enum(VALID_THEMES).optional(),
  chosen_style_theme: z.enum(VALID_THEMES).optional(),
  interaction_mode: z.enum(VALID_INTERACTION_MODES).optional(),
  inferred_interaction_mode: z.enum(VALID_INTERACTION_MODES).optional(),
  chosen_interaction_mode: z.enum(VALID_INTERACTION_MODES).optional(),
  style_theme_reason: z.string().optional(),
  profile: z.object({
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.theme && !body.style_theme && !body.chosen_style_theme) {
      body.style_theme = body.theme;
      body.chosen_style_theme = body.theme;
    }
    ...
    const normalizedProfile = ((normalizedBuilderBio.D as Record<string, unknown>)
      .profile ?? {}) as Record<string, unknown>;
    const explicitTheme = style_theme !== "default" ? style_theme : undefined;
    const effectiveInferredMode = firstNonEmptyString(
      inferred_interaction_mode,
      interaction_mode,
      normalizedProfile.inferred_interaction_mode,
      normalizedProfile.interaction_mode
    );
    const effectiveChosenMode = firstNonEmptyString(
      chosen_interaction_mode,
      interaction_mode,
      normalizedProfile.chosen_interaction_mode,
      normalizedProfile.interaction_mode,
      effectiveInferredMode
    );
    const effectiveInferredTheme = firstNonEmptyString(
      inferred_style_theme,
      explicitTheme,
      normalizedProfile.inferred_style_theme,
      normalizedProfile.style_theme
    );
    const effectiveChosenTheme = firstNonEmptyString(
      chosen_style_theme,
      explicitTheme,
      normalizedProfile.chosen_style_theme,
      normalizedProfile.style_theme,
      effectiveInferredTheme
    );
    if (effectiveInferredMode) {
      normalizedProfile.inferred_interaction_mode = effectiveInferredMode;
    }
    if (effectiveChosenMode) {
      normalizedProfile.chosen_interaction_mode = effectiveChosenMode;
      normalizedProfile.interaction_mode = effectiveChosenMode;
    }
    if (effectiveInferredTheme) {
      normalizedProfile.inferred_style_theme = effectiveInferredTheme;
    }
    if (effectiveChosenTheme) {
      normalizedProfile.chosen_style_theme = effectiveChosenTheme;
      normalizedProfile.style_theme = effectiveChosenTheme;
    }
    if (style_theme_reason) {
      normalizedProfile.style_theme_reason = style_theme_reason;
    }
    (normalizedBuilderBio.D as Record<string, unknown>).profile = normalizedProfile;
    ...
    if (existingUser) {
      ...
      await db
        ...
        .set({
          ...
          styleTheme: effectiveChosenTheme || explicitTheme || style_theme,
