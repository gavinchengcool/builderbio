...
- if Claude Code transcript logs start late, recover presence-only activity from trustworthy sidecar traces such as `~/.claude/todos` and `~/.claude/telemetry` rather than pretending the history starts later
